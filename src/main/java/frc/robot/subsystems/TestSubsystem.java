...
public class TestSubsystem extends SubsystemBase {
    ...
    public boolean atSpeed() {
        if (talon1.getVelocity().getValueAsDouble() < 5)
            return false;
        else if (MathUtil.isNear(talon1.getVelocity().getValueAsDouble(), targetSpeed, 0.8))
            return true;
        return false;
    }
    ...
    public boolean atSpeed(double tolerance) {
        if (talon1.getVelocity().getValueAsDouble() < 5)
            return false;
        else if (MathUtil.isNear(talon1.getVelocity().getValueAsDouble(), targetSpeed, tolerance))
            return true;
        return false;
    }
    ...
    public void angleShoot(double leftVelocity, double rightVelocity) {
        ...
        // if (leftVelocity > rightVelocity && Math.min(leftVelocity, rightVelocity) <
        // 30.0) {
        // v.Slot = 0;
        // talon1.setControl(v.withVelocity(leftVelocity));
        // v.Slot = 1;
        // talon2.setControl(v.withVelocity(rightVelocity));
        // } else if (leftVelocity < rightVelocity && Math.min(leftVelocity,
        // rightVelocity) < 30.0){
        // v.Slot = 1;
        // talon1.setControl(v.withVelocity(leftVelocity));
        // v.Slot = 0;

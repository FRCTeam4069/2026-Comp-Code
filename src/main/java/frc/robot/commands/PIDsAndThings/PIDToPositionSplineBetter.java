...
public class PIDToPositionSplineBetter extends Command {
    ...
    private static final double toleranceDegrees = 15.0;
    ...
    public PIDToPositionSplineBetter(SwerveDrivetrain drive,  ArrayList<Pose2d> waypoints, ArrayList<Double> tolerances, ArrayList<Boolean> stopAt) {
    ...
    public PIDToPositionSplineBetter(SwerveDrivetrain drive, ArrayList<Pose2d> waypoints, ArrayList<Double> tolerances, ArrayList<Boolean> stopAt, DrivetrainPIDConstants stopPointConstants, DrivetrainPIDConstants contPointConstants) {
        this.drive = drive;
    ...
    public void execute() {
        ...
        if ((distance < currentTolerance && (!currentStopAt || (Math.abs(drive.getPose().getRotation().getDegrees() - currentTarget.getRotation().getDegrees()) <= toleranceDegrees))) ){
            if (waypointIndex < waypoints.size() - 1 ){
                ...
                if(!currentStopAt) {
                    ...
                    ChassisSpeeds difference = new ChassisSpeeds(currentTarget.getX() - currentPose.getX(), currentTarget.getY() - currentPose.getY(), 0.0);
                    difference.vxMetersPerSecond /= distance;
                    difference.vyMetersPerSecond /= distance;
                    
                    difference.vxMetersPerSecond *= DrivetrainConstants.maxVelocity;
                    difference.vyMetersPerSecond *= DrivetrainConstants.maxVelocity;
                            
                    speeds = contPointController.calculate(drive.getPose(), currentTarget);
                    speeds.vxMetersPerSecond = difference.vxMetersPerSecond;
                    speeds.vyMetersPerSecond = difference.vyMetersPerSecond;
                }
